...
describe('is-https', () => {
  const tests = [
    ...
    {
      description: 'returns undefined if `x-forwarded-proto` header is not `https` and `req.connection.ecrypted` is falsy',
  ...
  for (const { description, input, output } of tests) {
    it(description, () => {
      expect(isHTTPS(...input as unknown as [ IncomingMessage, boolean ])).eq(output)
    })
  }
})
